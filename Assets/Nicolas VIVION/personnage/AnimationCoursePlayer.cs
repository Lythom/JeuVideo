...
namespace vivion {
    ...
    public class AnimationCoursePlayer : MonoBehaviour {
        ...
        public void SetAnimationFromSpeed (float vitesse, int axeY) {
            ...
            if (vitesse != 0 && currentAnim != GetAnimIndex ("RunTop") && axeY > 0) {
                ChangeAnimation ("RunTop");
            }
            if (vitesse != 0 && currentAnim != GetAnimIndex ("RunBot") && axeY < 0) {
                ChangeAnimation ("RunBot");
            }
            // Si le personnage se déplace et qu'il n'est pas déjà en train de courir, le faire courir
            if (vitesse != 0 && currentAnim != GetAnimIndex ("Run") && axeY == 0) {
                ChangeAnimation ("Run");
            }
